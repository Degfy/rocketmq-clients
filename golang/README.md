...
Project github.com/Degfy/rocketmq-clients/golang is targeted to implement with golang. At the current moment, it is still a work-in-progress project. Do not use it in production till it grows mature enough.
...
```go
import "github.com/Degfy/rocketmq-clients/golang"
```

Otherwise, to install the `golang` package, run the following command:

```sh
go get -u github.com/Degfy/rocketmq-clients/golang
```
